...
So, in practice, my final configuration consists of 2 magnets placed at a distance of few millimiters from the Hall effect sensor. The north pole of magnets is placed near the labeled side of the sensor, so the output voltage is low when the door is closed. However, I tested the system in order to correctly work at a distance of at most 3 centimeters. With this configuration, I avoided False Positives. Furthermore, considering protoype dimensions, when the magnets are placed at a distance of at most 2 centimeters from the Hall effect sensor, the opening angle is at most 5°, so it is not compatible with a break-in attempt.
